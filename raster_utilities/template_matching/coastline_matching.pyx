...
cpdef MatchToCoastline_long(long long[:,::1] data, char[:,::1] landMask, long long _NDV = -9999,
    ...
    This version is for long integer type input data.
    
    The output image is modified such that so that the data has values in and only in the
    areas marked in the mask. This is akin to the ArcGIS operations called "Clip" and "Nibble".
    ...
    useNearestNPixels parameter. By default n=1 to just use the nearest value, which is probably 
    what you want for integer data.
    ...
    # Generate the table of coordinate pairs which, when followed in order,
    # specify the offsets for a spiral search pattern.
...
cpdef MatchToCoastline_Float(float[:,::1] data, char[:,::1] landMask, float _NDV = -9999,
    ...
    This version is for float type input data.
    ...
    Gaps are filled with the mean of the nearest n data-containing pixels, where n is the
    useNearestNPixels parameter. By default n=1 to just use the nearest value, which may or
    may not be what you want for float data.
    These data pixels must be found within the specified searchPixelRadius. If fewer pixels are
    found within the specified radius,  a fill is still created if at least one is found.
    ...
    a fill for adjacent pixels. This will lead to "smearing" (down and right) and so this algorithm
    isn't appropriate for filling large areas, and generally it isn't appropriate for continuous
    data of any kind, hence the code originally being written for an integer input datatype. This 
    float version is provided as a convenience for edge cases where there are just a few pixels along 
    a coastline that need filling. Caveat emptor.
...
cpdef MatchToCoastline_Population(float[:,::1] data, char[:,::1] landMask, float _NDV = -9999,
